# Cross-Topic Mix Set 120

Coverage Window: topics 001 to 120
Newly Added Since Last Mix: topics 118 to 120
...
### Q1 (Mix 2 Concepts)
Concept Mix: Virtual Private Cloud + Bigtable
...
### Q2 (Mix 3 Concepts)
Concept Mix: Cloud Load Balancing + Resource Hierarchy Projects Iam + Private Vm Cloud Nat Lab
Scenario: You are deploying a production workload that combines the concepts above. The system must be secure, scalable, and cost-aware while minimizing operations overhead. What is the best approach?

A. Use broad Owner/Editor roles and one shared manual setup to reduce initial effort.
B. Optimize only for short-term cost and ignore latency, reliability, and recovery constraints.
C. Use correct load balancer type with DNS and edge strategy aligned to traffic pattern; project-level isolation for IAM, API enablement, quotas, and billing; private connectivity with HA VPN or Interconnect and correct egress design, and enforce least-privilege IAM with automated monitoring and alerts.
D. Rely on ad-hoc scripts with public exposure defaults and fix controls later.

Answer: C
Trap: The wrong options either over-privilege access, over-index on one constraint, or increase manual operational risk.
### Q3 (Mix 4 Concepts)
Concept Mix: Storage Overview + Gke Course Intro + Prompt Engineering Guide + Iam Roles
Scenario: You are deploying a production workload that combines the concepts above. The system must be secure, scalable, and cost-aware while minimizing operations overhead. What is the best approach?

A. Use storage class and lifecycle policy selected by access frequency and retention; GKE workload placement, rolling updates, and node pool optimization; managed-service-first architecture with least-privilege controls, and enforce least-privilege IAM with automated monitoring and alerts.
...
Scenario Focus: Cross-Topic Mix Set 120
...
Two designs both satisfy the happy path for Cross-Topic Mix Set 120. Which choice is most correct?
...
What should you validate first before choosing an architecture for Cross-Topic Mix Set 120?
