# Cross-Topic Mix Set 105

Coverage Window: topics 001 to 105
Newly Added Since Last Mix: topics 103 to 105

Focus Topics For This Set:
- Compute Engine Common Actions
- Minecraft Server Lab
- Iam Intro
- Iam Overview
- Iam Organization And Folders
- Iam Roles
- Iam Custom Role Demo
...
### Q1 (Mix 2 Concepts)
Concept Mix: Virtual Private Cloud + Kubernetes Object Model Declarative Management
...
### Q2 (Mix 3 Concepts)
Concept Mix: Cloud Load Balancing + Gke Course Cloud Intro + Projects And Billing
Scenario: You are deploying a production workload that combines the concepts above. The system must be secure, scalable, and cost-aware while minimizing operations overhead. What is the best approach?

A. Use broad Owner/Editor roles and one shared manual setup to reduce initial effort.
B. Optimize only for short-term cost and ignore latency, reliability, and recovery constraints.
C. Use correct load balancer type with DNS and edge strategy aligned to traffic pattern; GKE workload placement, rolling updates, and node pool optimization; project-level isolation for IAM, API enablement, quotas, and billing, and enforce least-privilege IAM with automated monitoring and alerts.
D. Rely on ad-hoc scripts with public exposure defaults and fix controls later.

Answer: C
Trap: The wrong options either over-privilege access, over-index on one constraint, or increase manual operational risk.
### Q3 (Mix 4 Concepts)
Concept Mix: Storage Overview + Dataproc + Firestore + Compute Engine Vm Creation Console
Scenario: You are deploying a production workload that combines the concepts above. The system must be secure, scalable, and cost-aware while minimizing operations overhead. What is the best approach?

A. Use storage class and lifecycle policy selected by access frequency and retention; analytics pipeline with correct ingest, transform, and warehouse roles; database choice by data model, scale pattern, and consistency requirements, and enforce least-privilege IAM with automated monitoring and alerts.
...
Trap: The wrong options either over-privilege access, over-index on one constraint, or increase manual operational risk.
### Q4 (Mix 5 Concepts)
Concept Mix: Choosing Hybrid Connectivity + Bigquery + Gke Overview + Jenkins Marketplace Lab + Compute Engine Disk Options
Scenario: You are deploying a production workload that combines the concepts above. The system must be secure, scalable, and cost-aware while minimizing operations overhead. What is the best approach?

A. Use broad Owner/Editor roles and one shared manual setup to reduce initial effort.
B. Optimize only for short-term cost and ignore latency, reliability, and recovery constraints.
C. Rely on ad-hoc scripts with public exposure defaults and fix controls later.
D. Use private connectivity with HA VPN or Interconnect and correct egress design; analytics pipeline with correct ingest, transform, and warehouse roles; GKE workload placement, rolling updates, and node pool optimization, and enforce least-privilege IAM with automated monitoring and alerts.
...
#### Q1
Scenario Focus: Cross-Topic Mix Set 105
...
#### Q6
Scenario Focus: Cross-Topic Mix Set 105
Two designs both satisfy the happy path for Cross-Topic Mix Set 105. Which choice is most correct?
...
#### Q7
Scenario Focus: Cross-Topic Mix Set 105
What should you validate first before choosing an architecture for Cross-Topic Mix Set 105?
