# Cross-Topic Mix Set 081

Coverage Window: topics 001 to 081
Newly Added Since Last Mix: topics 079 to 081
...
- Custom Subnet Expansion Lab

Question Count: 4
Each question mixes 2 to 5 previously covered concepts.

### Q1 (Mix 2 Concepts)
Concept Mix: Virtual Private Cloud + Cloud Computing Overview Gke Course
...
B. Use managed-service-first architecture with least-privilege controls; GKE workload placement, rolling updates, and node pool optimization, and enforce least-privilege IAM with automated monitoring and alerts.
...
Trap: The wrong options either over-privilege access, over-index on one constraint, or increase manual operational risk.
### Q3 (Mix 4 Concepts)
Concept Mix: Storage Overview + Cloud Marketplace + What Is A Container + Architecting Intro
Scenario: You are deploying a production workload that combines the concepts above. The system must be secure, scalable, and cost-aware while minimizing operations overhead. What is the best approach?

A. Use storage class and lifecycle policy selected by access frequency and retention; infrastructure-as-code and standardized deployment templates; GKE workload placement, rolling updates, and node pool optimization, and enforce least-privilege IAM with automated monitoring and alerts.
...
Trap: The wrong options either over-privilege access, over-index on one constraint, or increase manual operational risk.
### Q4 (Mix 5 Concepts)
Concept Mix: Choosing Hybrid Connectivity + Terraform + Google Cloud Network And Infrastructure + Firestore + Custom Subnet Expansion Lab
Scenario: You are deploying a production workload that combines the concepts above. The system must be secure, scalable, and cost-aware while minimizing operations overhead. What is the best approach?

A. Use broad Owner/Editor roles and one shared manual setup to reduce initial effort.
B. Optimize only for short-term cost and ignore latency, reliability, and recovery constraints.
C. Rely on ad-hoc scripts with public exposure defaults and fix controls later.
D. Use private connectivity with HA VPN or Interconnect and correct egress design; infrastructure-as-code and standardized deployment templates; managed-service-first architecture with least-privilege controls, and enforce least-privilege IAM with automated monitoring and alerts.
...
#### Q1
Scenario Focus: Cross-Topic Mix Set 081
...
#### Q6
Scenario Focus: Cross-Topic Mix Set 081
Two designs both satisfy the happy path for Cross-Topic Mix Set 081. Which choice is most correct?
...
#### Q7
Scenario Focus: Cross-Topic Mix Set 081
What should you validate first before choosing an architecture for Cross-Topic Mix Set 081?
