# Cross-Topic Mix Set 090

Coverage Window: topics 001 to 090
Newly Added Since Last Mix: topics 088 to 090

Focus Topics For This Set:
- Vm Internal External Ip Lab
- Dns External Mapping And Alias Ips
- Routes And Firewall Rules
- Network Pricing
- Vpc Network Lab Connectivity
- Common Network Designs
- Private Vm Cloud Nat Lab
- Compute Engine Vm Overview

Question Count: 4
Each question mixes 2 to 5 previously covered concepts.

### Q1 (Mix 2 Concepts)
Concept Mix: Virtual Private Cloud + Billing And Cost Controls
...
B. Use managed-service-first architecture with least-privilege controls; budgets and alerts with quotas plus billing export visibility, and enforce least-privilege IAM with automated monitoring and alerts.
...
### Q2 (Mix 3 Concepts)
Concept Mix: Cloud Load Balancing + Dataproc Lab Walkthrough + Gke
Scenario: You are deploying a production workload that combines the concepts above. The system must be secure, scalable, and cost-aware while minimizing operations overhead. What is the best approach?

A. Use broad Owner/Editor roles and one shared manual setup to reduce initial effort.
B. Optimize only for short-term cost and ignore latency, reliability, and recovery constraints.
C. Use correct load balancer type with DNS and edge strategy aligned to traffic pattern; analytics pipeline with correct ingest, transform, and warehouse roles; GKE workload placement, rolling updates, and node pool optimization, and enforce least-privilege IAM with automated monitoring and alerts.
D. Rely on ad-hoc scripts with public exposure defaults and fix controls later.

Answer: C
Trap: The wrong options either over-privilege access, over-index on one constraint, or increase manual operational risk.
### Q3 (Mix 4 Concepts)
Concept Mix: Storage Overview + Managed Services Intro + Gke Kubernetes Concepts Module Intro + Vpc Objects Overview
Scenario: You are deploying a production workload that combines the concepts above. The system must be secure, scalable, and cost-aware while minimizing operations overhead. What is the best approach?

A. Use storage class and lifecycle policy selected by access frequency and retention; analytics pipeline with correct ingest, transform, and warehouse roles; GKE workload placement, rolling updates, and node pool optimization, and enforce least-privilege IAM with automated monitoring and alerts.
...
Trap: The wrong options either over-privilege access, over-index on one constraint, or increase manual operational risk.
### Q4 (Mix 5 Concepts)
Concept Mix: Choosing Hybrid Connectivity + Cloud Storage Classes + Container Images And Cloud Build + Cloud Run + Private Vm Cloud Nat Lab
Scenario: You are deploying a production workload that combines the concepts above. The system must be secure, scalable, and cost-aware while minimizing operations overhead. What is the best approach?

A. Use broad Owner/Editor roles and one shared manual setup to reduce initial effort.
B. Optimize only for short-term cost and ignore latency, reliability, and recovery constraints.
C. Rely on ad-hoc scripts with public exposure defaults and fix controls later.
D. Use private connectivity with HA VPN or Interconnect and correct egress design; storage class and lifecycle policy selected by access frequency and retention; Compute Engine design with autoscaling, resilience, and right sizing, and enforce least-privilege IAM with automated monitoring and alerts.
...
#### Q1
Scenario Focus: Cross-Topic Mix Set 090
...
#### Q6
Scenario Focus: Cross-Topic Mix Set 090
Two designs both satisfy the happy path for Cross-Topic Mix Set 090. Which choice is most correct?
...
#### Q7
Scenario Focus: Cross-Topic Mix Set 090
What should you validate first before choosing an architecture for Cross-Topic Mix Set 090?
