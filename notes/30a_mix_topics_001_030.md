# Cross-Topic Mix Set 030

Coverage Window: topics 001 to 030
Newly Added Since Last Mix: topics 028 to 030
...
- Load Balancing Module Summary
- Infrastructure Automation Intro
- Terraform
...
### Q1 (Mix 2 Concepts)
Concept Mix: Virtual Private Cloud + Application Load Balancing
...
### Q2 (Mix 3 Concepts)
Concept Mix: Cloud Load Balancing + Mig Autoscaling Health Checks + Infrastructure Automation Intro
Scenario: You are deploying a production workload that combines the concepts above. The system must be secure, scalable, and cost-aware while minimizing operations overhead. What is the best approach?

A. Use broad Owner/Editor roles and one shared manual setup to reduce initial effort.
B. Optimize only for short-term cost and ignore latency, reliability, and recovery constraints.
C. Use correct load balancer type with DNS and edge strategy aligned to traffic pattern; regional MIG with health checks, autohealing, and autoscaling policy; infrastructure-as-code and standardized deployment templates, and enforce least-privilege IAM with automated monitoring and alerts.
D. Rely on ad-hoc scripts with public exposure defaults and fix controls later.

Answer: C
Trap: The wrong options either over-privilege access, over-index on one constraint, or increase manual operational risk.
### Q3 (Mix 4 Concepts)
Concept Mix: Storage Overview + Managed Instance Groups + Network Load Balancing + Cloud Computing Overview
Scenario: You are deploying a production workload that combines the concepts above. The system must be secure, scalable, and cost-aware while minimizing operations overhead. What is the best approach?
...
Trap: The wrong options either over-privilege access, over-index on one constraint, or increase manual operational risk.
### Q4 (Mix 5 Concepts)
Concept Mix: Choosing Hybrid Connectivity + Alb Examples Cross Region Content Based + Choosing Load Balancer + Virtual Private Cloud + Cloud Interconnect Dedicated Partner Crosscloud
Scenario: You are deploying a production workload that combines the concepts above. The system must be secure, scalable, and cost-aware while minimizing operations overhead. What is the best approach?
...
#### Q1
Scenario Focus: Cross-Topic Mix Set 030
...
#### Q6
Scenario Focus: Cross-Topic Mix Set 030
Two designs both satisfy the happy path for Cross-Topic Mix Set 030. Which choice is most correct?
...
#### Q7
Scenario Focus: Cross-Topic Mix Set 030
What should you validate first before choosing an architecture for Cross-Topic Mix Set 030?
