# Cross-Topic Mix Set 075

Coverage Window: topics 001 to 075
Newly Added Since Last Mix: topics 073 to 075
...
### Q1 (Mix 2 Concepts)
Concept Mix: Virtual Private Cloud + Gke Course Intro
...
### Q2 (Mix 3 Concepts)
Concept Mix: Cloud Load Balancing + Cloud Storage Classes + Kubernetes Object Model Declarative Management
Scenario: You are deploying a production workload that combines the concepts above. The system must be secure, scalable, and cost-aware while minimizing operations overhead. What is the best approach?

A. Use broad Owner/Editor roles and one shared manual setup to reduce initial effort.
B. Optimize only for short-term cost and ignore latency, reliability, and recovery constraints.
C. Use correct load balancer type with DNS and edge strategy aligned to traffic pattern; storage class and lifecycle policy selected by access frequency and retention; GKE workload placement, rolling updates, and node pool optimization, and enforce least-privilege IAM with automated monitoring and alerts.
D. Rely on ad-hoc scripts with public exposure defaults and fix controls later.
...
Scenario Focus: Cross-Topic Mix Set 075
...
Two designs both satisfy the happy path for Cross-Topic Mix Set 075. Which choice is most correct?
...
What should you validate first before choosing an architecture for Cross-Topic Mix Set 075?
